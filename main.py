...
class TaskCreationChain(LLMChain):
  ...
  def get_next_task(self, result: Dict, task_description: str,
                    task_list: List[str]) -> List[Dict]:
    """Get the next task."""
    incomplete_tasks = ", ".join(task_list)
    response = self.run(result=result,
...
class TaskPrioritizationChain(LLMChain):
  ...
  def prioritize_tasks(self, this_task_id: int,
                       task_list: List[Dict]) -> List[Dict]:
    """Prioritize tasks."""
...
class BabyAGI(BaseModel):
  ...
  def print_task_ending(self):
    with Message(label="Task Ending") as m:
      m.write("### You've Reached Your 5 Tasks")
      m.write("")
  ...
  def run(self, max_iterations: Optional[int] = None):
    ...
    while True:
      if self.task_list:
        ...
        result = self.execution_chain.execute_task(self.objective,
                                                   task["task_name"])
...
def main():
  ...
  st.write("this is an experimental project use at your own risk")
  ...
  if button:
    try:
      baby_agi = BabyAGI.from_llm_and_objectives(llm=OpenAI(
        openai_api_key=openai_api_key, model_name="gpt-3.5-turbo"),
                                                 vectorstore=vectorstore,
                                                 objective=objective,
